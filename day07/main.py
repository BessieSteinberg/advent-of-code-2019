...
def amp_output(amp_settings, program):
    """
    ...
    for amp_setting in amp_settings:
        inputs = [amp_setting] + amp_outputs
        int_computer = IntComputer(program)
        amp_outputs = int_computer.run_program(inputs)

    return amp_outputs[0]


def best_amp_setting_sequence(program):
    """
    ...
    highest_output = 0
    best_sequence = []
    for setting_sequence in permutations(range(5)):
        output = amp_output(list(setting_sequence), program)
        if highest_output < output:
            highest_output = output
            best_sequence = setting_sequence
